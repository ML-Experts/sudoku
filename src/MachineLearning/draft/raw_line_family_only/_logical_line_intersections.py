...
class _SegmentPairIntersectionCandidate:
    ...
    def selection_key(self) -> tuple[int, int, float]:
        cross_count = int(
            self.horizontal_kind == LogicalLineIntersectionKind.CROSS
        ) + int(self.vertical_kind == LogicalLineIntersectionKind.CROSS)
        return (
            2 - cross_count,
...
def _classify_logical_line_order(
    ...
    if axis_value == logical_line.axis_start and axis_value == logical_line.axis_end:
        return IntersectionOrder.BOTH
    if axis_value == logical_line.axis_start:
        return IntersectionOrder.START
    if axis_value == logical_line.axis_end:
        return IntersectionOrder.END
    return IntersectionOrder.MIDDLE
...
def _build_segment_pair_candidate(
    ...
    horizontal_kind = (
        ...
        if horizontal_segment_order == IntersectionOrder.MIDDLE
        else LogicalLineIntersectionKind.TOUCH
    )
    vertical_kind = (
        LogicalLineIntersectionKind.CROSS
        if vertical_segment_order == IntersectionOrder.MIDDLE
        else LogicalLineIntersectionKind.TOUCH
    )
    return _SegmentPairIntersectionCandidate(
        ...
        point=rounded_point,
        horizontal_kind=horizontal_kind,
        ...
        for vertical_line in vertical_logical_lines:
            ...
            logical_line_intersections.append(
                LogicalLineIntersection(
                    ...
                    horizontal_kind=best_candidate.horizontal_kind,
                    vertical_kind=best_candidate.vertical_kind,
                    horizontal_order=_classify_logical_line_order(
                        best_candidate.horizontal_line,
                        best_candidate.point,
                    ),
                    vertical_order=_classify_logical_line_order(
                        best_candidate.vertical_line,
                        best_candidate.point,
                    ),
                )
            )
