...
## Krok 5a. Containment prune

To jest osobny etap wykonywany po grouping `RAW` i przed pixel connection.

Wejście:

- lista `LogicalLine` jednej rodziny po grouping `RAW`,
- `pixel_connection_binary`
...
Etap próbuje usunąć te `LogicalLine`, które:

- są zawarte na osi głównej wewnątrz dłuższej linii tej samej rodziny,
- leżą bardzo blisko niej na osi poprzecznej lub da się do niej dojść po białych
  pikselach,
- nie wnoszą dodatkowej wartości do dalszych etapów connection i intersections.

### Najważniejsze modele i wynik
...
Każdy `LogicalLineCrossAxisGroup` zawiera:
...
2. najdłuższa linia staje się `container_line`
3. zbierane są linie zawarte w jej `axis_start..axis_end`
4. kandydaci są grupowani po ciągłości osi poprzecznej
5. ciągłość może być uznana:
   - przez mały dystans na osi poprzecznej,
   - przez overlap końca lub początku zakresu osi poprzecznej,
   - przez znalezienie ścieżki po białych pikselach od końca kandydata do
     `container_line`
6. linie uznane za zawarte trafiają do `removed_logical_lines`
7. pozostałe wracają do puli i mogą zostać później wybrane jako osobne
   `container_line`

### Stan po containment prune
...
To nie jest kolejny snapshot listy linii jak `pre_connection` i `post_connection`,
ale pełny wynik diagnostyczny pruningu zawierający zarówno linie pozostawione,
jak i usunięte oraz ich grupowanie na osi poprzecznej.
...
Na wejściu connection trafiają już linie po containment prune, a nie pełna lista
linii ze stanu `pre_connection`.
...
Ten sam moduł zawiera też helper point-to-line używany przez containment prune do
sprawdzania, czy kandydat można po białych pikselach połączyć z `container_line`
w ograniczonym prostokącie tolerancji.
...
flowchart TD
    ...
    savePre --> containment[prune contained logical lines]
    containment --> pixelConnect[connect_logical_lines_by_pixels]
...
5. Pomiędzy `pre_connection` i `post_connection` istnieje osobny wynik
   containment prune, który nie jest tylko listą linii, ale pełnym artefaktem
   diagnostycznym.
6. Kod traktuje `raw_line_family_only_logical_lines.py` jako publiczną fasadę,
