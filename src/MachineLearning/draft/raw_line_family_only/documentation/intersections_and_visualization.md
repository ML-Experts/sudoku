...
- `container_line` pozostawione po containment prune,
- `removed_logical_lines` usunięte jako linie zawarte,
- grupowanie po ciągłości osi poprzecznej,
- stan pomiędzy grouping `RAW` a pixel connection.

Board containment prune używa linii pozostawionych po pruningu, a overlaye
dodatkowo wyróżniają usunięte linie oraz etykiety `rm=...` dla każdego
kontenera.

### 4. Logical lines po connection
...
- wynik containment prune dla rodzin poziomych i pionowych,
