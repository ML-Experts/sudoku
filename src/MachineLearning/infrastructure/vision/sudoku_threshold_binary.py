...
def build_repair_variants(
    ...
    horizontal_closed = close_binary_image(
        binary_image,
        (directional_kernel_length, 1),
    )
    vertical_closed = close_binary_image(
        binary_image,
        (1, directional_kernel_length),
    )
...
def prepare_board_binary_for_frame_detection(
    binary_image: np.ndarray,
    config: SudokuThresholdConfig,
) -> np.ndarray:
    _, cleanup_variants = build_cleanup_variants(binary_image, config)
    cleanup_variant_name = config.selected_cleanup_variant or "adaptive_only"
    cleaned_binary = cleanup_variants.get(cleanup_variant_name, binary_image)
    if not np.any(cleaned_binary):
        cleaned_binary = binary_image.copy()

    repair_variants = build_repair_variants(cleaned_binary, config)
    repair_variant_name = config.selected_repair_variant or "cleanup_only"
    repaired_binary = repair_variants.get(repair_variant_name, cleaned_binary)
    if not np.any(repaired_binary):
        return cleaned_binary

    return repaired_binary
...
__all__ = [
    ...
    "close_binary_image",
    "open_binary_image",
    "prepare_board_binary_for_frame_detection",
    "remove_small_connected_components",
